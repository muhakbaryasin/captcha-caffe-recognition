...
def main():
	...
	while global_config['iteration_number'] < 21 and global_config['all_correct_times'] < 3:
		...
		if last_snapshot is None:
			...
			subcall(["cp", "-r", "temp/hash2_prep", train_files_dir])
			...
			subcall(["rm", "-rf", test_files_dir])
			# subcall(["cp", "-r", "temp/labeled/" + str(global_config['iteration_number']), test_files_dir])
			subcall(["cp", "-r", "temp/hash2_prep", test_files_dir])
			...
			for idx in xrange(init_task):
				...
				list_of_test_task[idx] = classifyImage.delay(correct_string, image_file_path, "network_captchas_with_3_convolutional_layers_lion.prototxt", snapshots_dir+last_snapshot['caffemodel'])
				queue_no += 1
